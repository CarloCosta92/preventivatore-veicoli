    ...
    return (
        <>
            <div className="text-center p-5">
                <h1 className="font-bold text-3xl text-text-default w-full mb-5">I Tuoi Preventivi</h1>
                <div className="mx-5">
                    {quotations.length > 0 ? <div>lista dei tuoi preventivi</div> : <h3 className="text-2xl font-bold">Ancora nessun Preventivo</h3>}
                </div>
                <button className="default-btn m-3" onClick={() => setIsClicked(prev => !prev)}>Visualizza i tuoi Preventivi</button>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-default backdrop-blur-sm">
                    <form onSubmit={(e) => handleSubmit(e)}>
                        <div className="flex justify-between">
                            <label className="text-sm font-medium mb-2" htmlFor="email">Inserisci la tua mail</label>
                            {/* Aggiunto type="button" per evitare che invii il form */}
                            <button type="button" className="text-red-800 hover:cursor-pointer hover:text-red-400 hover:scale-110" onClick={() => setIsClicked(prev => !prev)}>
                                <CircleX />
                            </button>
                        </div>
                        <div className="text-center p-3">
                            ...
                            {error.email && <div className="text-red-800 text-sm m-1">{error.email}</div>}
